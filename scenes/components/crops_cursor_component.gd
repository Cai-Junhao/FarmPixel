...
func _unhandled_input(event: InputEvent) -> void:
	if event.is_action("remove_dirt"):
...
func add_crops() -> void:
	if distance < 20.0:
		if ToolManager.selected_tool == DataTypes.Tools.PlantCorn:
			...
			corn_instance.globel_position = local_cell_position
			get_parent().find_child("CropFields").add_child(corn_instance)
		
		if ToolManager.seleted_tool == DataTypes.Tools.PlantTomato:
			var tomato_instance = corn_plant_scene.instantiate() as Node2D
			tomato_instance.globel_position = local_cell_position
